// Basicamente pra gerar um serial ele pega quatro dígitos aleatórios, cuja a soma deles, vezes 10 seja divisível por 11.
...
const GerarLetras = qtd => {
    const letras = Array(qtd).fill(0).reduce(nums => {
...
const validaChaveL = _ => {
    ...
    const Validação = PrimeiraChave.reduce((acc, att) => acc + att)
    return Validação % 11 === 0 ? PrimeiraChave.map(val => String.fromCharCode(val)).sort() : validaChaveL()
}
...
// Depois pega uma letra, entre A = 65 e Z = 90, cuja a soma dos valores associados a ele, multiplicados por 10 seja divisível por 11,

// e mais 4 dígitos aleatórios, cuja a soma deles, vezes 10, seja divisível por 11.

// Aí, ele gera a serial da seguinte forma:

// XXXAAAABBBBCCCC onde XXX não é validado, serve apenas para identificar o produto ao qual se refere a serial (e.g. PDV para o caixa, ERP para o ERP),
